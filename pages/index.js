...
export default function Home() {
  ...
  useEffect(() => {
    // Inicjalizacja Supabase tylko po stronie przeglądarki
  ...
  return (
    <div style={{ padding: '40px', background: '#000', minHeight: '100vh', color: '#fff', fontFamily: 'sans-serif' }}>
      ...
      {loading ? (
        ...
        <button 
          onClick={login} 
          style={{ padding: '15px 30px', cursor: 'pointer', background: '#5865F2', border: 'none', color: '#fff', borderRadius: '5px' }}
        >
          Zaloguj przez Discord
        </button>
      ) : (
        <div>
          <h2>Witaj, {user.user_metadata?.full_name || 'Graczu'}</h2>
